...
const TopSelling = () => {
  ...
  const getTopItem = async () => {
    try {
      const res = await axios.get("/sale/top-selling-items");

      if (res.data.success) {
        const data = res.data.data;

        // Top item is always first in the list
        setTopItem(data[0]);

        // calculate total only if needed
  ...
  if (!topItem) return <div>No top item found</div>;
  ...
  return (
    <div className="p-5 bg-white border border-gray-100 rounded-2xl w-full max-w-sm">
      ...
      <div className="mt-3">
        <div className="flex items-center justify-between mb-1">
          <p className="text-gray-800 font-medium">{topItem.productName}</p>
          <p className="text-gray-600 text-sm">
